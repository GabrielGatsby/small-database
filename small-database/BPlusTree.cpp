...
//在B+树查询一个大于等于该查找关键字的结果（用于大于的范围查找）（别的函数请不要用）
void BPlusTree::Search_BPlus_Tree_Bigger(TRecord &trecord, TSearchResult &tresult) {
	...
	do {
		...
		if (i < a.nkey && a.isleaf && trecord.key == a.keys[i]) {		//在B+树叶节点找到了等值的关键字
			...
			for (i = 0; trecord.key > a.keys[i]; i++);//定位到第一个大于查找关键字的关键字
			tresult.Baddress = current;
	...
	if (i < x.nkey && x.keys[i] == trecord.key) {	//在当前节点找到关键字
		...
		if (!x.isleaf) {	  //在内节点找到关键字
			...
			if (child.isleaf) {	 //如果孩子是叶节点
				...
				KEYTYPE should_remember = x.keys[i];//记录原数据
...
void	BPlusTree::Delete_BPlus_Tree(TRecord &trecord)	//在B+中删除一个关键字
{
...
void BPlusTree::Delete_BPlus_Tree_Address_Under_Key(TRecord & trecord)//删除指定关键字下的指定记录地址，此trecord中包含查找的关键字和要删除的记录地址
{
	...
	if (!tresult.exist) {
		...
		else if (count == 1) {//当count==1时，索引作用失去，删除索引
			TRecord temprecord;
...
void  BPlusTree::EnumLeafKey()	//依次枚举B+树叶节点的所有关键字
{
	...
	while (1)
	{
		...
		if (head.pointers[MAX_KEY].second == 0)//没有下一个叶节点
			break;
